...
/**
 * In Java, it don't support unsigned int, so we use char to replace uint8. The range of byte is [-128,127], and the
 * range of char is [0,65535]. So the byte could used to store the uint8. (We assume that the String could be mapped to
 * assic)
 * 
 * @author afunx
 * 
...
public class ByteUtil
{
    ...
    /**
     * Split uint8 to 2 bytes of high byte and low byte. e.g. 20 = 0x14 should be split to [0x01,0x04] 0x01 is high byte
     * and 0x04 is low byte
     * 
     * @param uint8 the char(uint8)
     * @return the high and low bytes be split, byte[0] is high and byte[1] is low
     */
